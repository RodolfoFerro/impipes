...
modified_images = pipeline.run(save_files=False, return_list=True)      #process the selected image with the defined sequence of filters. Filtered image would not be stored (save_files=False) but will be returned as a [numpy.ndarrays] (return_list = True)
