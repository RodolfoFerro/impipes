...
class Filter(object):
	...
	def run(self):
		"""
		   ...
		   Contains code of the main image process of a filter. Should return a processed image.
	   """
...
class Gamma(Filter):
	...
	image : numpy.ndarray or path to an image file
		A NumPy's ndarray from cv2.imread as an input or path to an image file to be opened with cv2.imread.
...
class Kernel(Filter):
	...
	def run(self):
		if self.image is not None:
			kernel = np.matrix(self.kernel).tolist() if type(self.kernel) is str else self.kernel or [[1,1,1],[1,20,1],[1,1,1]]
			kernel_sum = 0
			for line in kernel:
				kernel_sum += sum(line)
...
class Denoise(Filter):
	...
	def run(self):
		if self.image is not None:
			self.filteredImage = cv2.fastNlMeansDenoisingColored(self.image,None,self.strength,self.strength,7,21)
...
class Dehaze(Filter):
	...
	def _get_dark_channel(self, img):
		"""
		internal method called from _get_transmission() method (not to be used out of Dehaze class)
	...
	def _get_atmospheric_light(self, img, dark):
		"""internal method called from run() method (not to be used out of Dehaze class)
...
class Unsharp(Filter):
	...
	def run(self):
		if self.image is not None:
			...
			for i in range(3):
				sharp[:,:,i] = self._unsharp_channel(self.img[:,:,i], self.sigma, self.ustrength)
...
class CLAHE(Filter):
	...
	def run(self):
		...
		if self.image is not None:		   
			he = cv2.createCLAHE(clipLimit=self.clip_limit, tileGridSize=(self.tile_grid_size, self.tile_grid_size))
